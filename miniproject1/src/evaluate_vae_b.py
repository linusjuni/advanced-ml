...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate Gaussian-decoder VAE (Part B) on standard MNIST")
    ...
    ckpt_dir = args.vae_checkpoint

    logger.info("Loading VAE...")
    model, config = load_model(ckpt_dir)
    assert isinstance(model, VAE)
    logger.info(f"Loaded VAE with config: {config}")
    ...
    for i, ax in enumerate(axes):
        ...
        ax.axis("off")
    fig.suptitle("VAE samples (Gaussian decoder)", fontsize=12)
    plt.tight_layout()
