...
def main():
    ...
    with torch.no_grad():
        four_samples = model.sample(4)
        four_samples = four_samples.view(4, 1, 28, 28).clamp(-1, 1)
        save_image(four_samples * 0.5 + 0.5, f"{args.output_dir}/vae_samples.png", nrow=4)
        print(f"Saved 4 samples to {args.output_dir}/vae_samples.png")
