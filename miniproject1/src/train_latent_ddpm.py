...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pretrained-vae-checkpoint", type=str, default=None, help="path to pretrained VAE checkpoint for latent diffusion")
    parser.add_argument("--M", type=int, default=32, help="latent dimension from VAE for latent diffusion")
...
def main():
    ...
    logger.info("Training Latent DDPM with U-Net:")
    for key, value in sorted(vars(args).items()):
        logger.info(f"  {key} = {value}")

    torch.manual_seed(args.seed)

    config = LatentDDPMConfig(T=args.T, M=args.M)
    model = _build_latent_ddpm(config).to(args.device)
