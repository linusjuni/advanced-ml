...
def compute_geodesic(x_start, x_end, decoder, n_points=10, n_steps=200):
    ...
    # Interpolate linearly as starting guess
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        ...
        sc = plt.scatter(zs[:, 0], zs[:, 1], c=ys, s=8, cmap="tab10", alpha=0.7)
        for start_idx, end_idx in indices:
            ...
            geodesic = compute_geodesic(x_start, x_end, model.decoder)

            geodesic_np = geodesic.detach().cpu().numpy()
            x_start_np = x_start.detach().cpu().numpy()
            x_end_np = x_end.detach().cpu().numpy()
            distance = torch.sqrt(((x_end - x_start) ** 2).sum()).item()
            # print(f"Geodesic distance between points {start_idx} and {end_idx}: {distance:.4f}")
            with open(args.experiment_folder + "/geodesic_distances.txt", "a") as f:
                f.write(f"Geodesic distance between points {start_idx} and {end_idx}: {distance:.4f}\n")
            ...
            # Mark the endpoints
            plt.scatter(*x_start_np, c='green', s=50, zorder=5)
            plt.scatter(*x_end_np, c='red', s=50, zorder=5)
        plt.colorbar(sc)
        plt.xlabel("z1")
        plt.ylabel("z2")
        plt.title("VAE Latent Space")
        plt.savefig(args.experiment_folder + "/latent_space.png")
        plt.show()
